...
//        }
    ...
    private static boolean isManaSourceReserved(Player ai, Card sourceCard, SpellAbility sa) {
        ...
        if (curPhase == PhaseType.MAIN2 || curPhase == PhaseType.CLEANUP) {
            aic.getCardMemory().clearMemorySet(LearnedAiCardMemory.MemorySet.HELD_MANA_SOURCES);
        }
        else {
            if (aic.getCardMemory().isRememberedCard(sourceCard, LearnedAiCardMemory.MemorySet.HELD_MANA_SOURCES)) {
