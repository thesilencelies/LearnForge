...
public class AttachAi extends SpellAbilityAi {
    ...
    private static Card attachToCardAIPreferences(final Player aiPlayer, final SpellAbility sa, final boolean mandatory) {
        ...
        if (c != null && attachSource.isEquipment()
            ...
            if (!decideMoveFromUseless && aic.getCardMemory().isMemorySetEmpty(LearnedAiCardMemory.MemorySet.HELD_MANA_SOURCES)) {
            ...
            if (aic.getCardMemory().isRememberedCard(sa.getHostCard(), LearnedAiCardMemory.MemorySet.ATTACHED_THIS_TURN)) {
                return null;
            }
        }

        aic.getCardMemory().rememberCard(sa.getHostCard(), LearnedAiCardMemory.MemorySet.ATTACHED_THIS_TURN);
